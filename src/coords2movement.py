...
class Coords2Movement:
    ...
    def compute_angles(self, coords):
        ...
        s0_r = np.pi - np.arccos(np.dot(l01[:2], l02[:2]) / (np.linalg.norm(l01[:2]) * np.linalg.norm(l02[:2])))
        s1_r = np.arctan2(l02[2], np.linalg.norm(l02[:2]))
        e0_r = np.pi / 2 + np.arctan2(l24[2], np.linalg.norm(l24[:2]))
        e1_r = np.arccos(np.dot(l02, l24) / (np.linalg.norm(l02) * np.linalg.norm(l24)))

        s0_l = np.pi - np.arccos(np.dot(-l01[:2], l13[:2]) / (np.linalg.norm(-l01[:2]) * np.linalg.norm(l13[:2])))
        s1_l = np.arctan2(l13[2], np.linalg.norm(l13[:2]))
        e0_l = -np.pi / 2 - np.arctan2(l35[2], np.linalg.norm(l35[:2]))
